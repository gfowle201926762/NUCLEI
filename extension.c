...
void shell_loop(parser* prsr, interpret* intrp)
{
   bool quit = false;
   char str[STRLEN] = {'\0'};
   while (!quit){
      printf(">>> ");
      fgets(str, sizeof(char) * STRLEN, stdin);
      clear_parser(prsr);
      strcpy(prsr->str, str);
      if (!is_instrct(prsr, intrp) && strcmp(str, "q\n")){
         printf("[ERROR] %s\n", prsr->trc);
      }
      if (!strcmp(str, "q\n")){
         quit = true;
...
void shell(void)
{
   ...
   printf("\nWelcome to the interactive NUCLEI shell.\
   \nType one INSTRCT at a time.\
   \nEnter q to quit.\n\n");
   shell_loop(prsr, intrp);
...
void init_funcs(interpret* intrp)
{
   ...
   intrp->func_ptr[show] = intrp_show;
}
#endif
...
void count(lisp* l, atomtype* accum)
{
   *accum = *accum + lisp_isatomic(l);
}

void non_atom(lisp* l, atomtype* accum)
{
   *accum = *accum + (1 - lisp_isatomic(l));
}
...
// draw out a lisp
void next_layer(layer* ly)
{
   //printf("START OF next_layer\n");

   ly->width = 0;
   int i = 0;
   while (ly->layers[ly->height][i]){
      lisp* p = ly->layers[ly->height][i];
      while (p){
         if (!lisp_isatomic(p)){
            ly->layers[ly->height + 1][ly->width] = lisp_car(p);
            ly->width += 1;
         }
         p = lisp_cdr(p);
      }
...
{
   //printf("START OF init_layer\n");

   layer* ly = calloc(1, sizeof(layer));
   if (ly == NULL){
...
void print_space(interpret* intrp, layer* ly)
{
   //printf("START OF print_space\n");

   while (ly->line_index < ly->now[ly->get_i]){
      ly->line_index += 1;
      strcat(intrp->istr, "     ");
   }
}
...
void print_atom(interpret* intrp, lisp* parent, char* s, layer* ly)
{
   //printf("START OF print_atom\n");

   print_space(intrp, ly);
   if (!strcmp(s, NODE_START)){
      char val[TRCLEN] = {'\0'};
      lisp_tostring(parent, val);
      int count = BLOCK - strlen(val);
      for (int i = 0; i < count; i++){
         strcat(intrp->istr, " ");
      }
      strcat(intrp->istr, val);
      strcat(intrp->istr, " ");
   }
   else{
      strcat(intrp->istr, "     ");
...
void print_car(interpret* intrp, char* s, layer* ly)
{
   //printf("START OF print_car\n");

   print_space(intrp, ly);
   strcat(intrp->istr, s);
   ly->prev[ly->prev_i] = ly->line_index;
   ly->prev_i += 1;
   ly->get_i += 1;
   ly->line_index += 1;
}
...
void print_cdr(interpret* intrp, char* g, char* c, layer* ly)
{
   //printf("START OF print_cdr\n");

   for (int i = 1; i < intrp->atom_a; i++){
      strcat(intrp->istr, g);
      ly->line_index += 1;
   }
   strcat(intrp->istr, c);
...
void print_line(interpret* intrp, lisp* parent, char* s, char* c, char* g, layer* ly)
{
   //printf("START OF print_line\n");

   if (lisp_isatomic(parent)){
      print_atom(intrp, parent, s, ly);
   }
   lisp* cdr = lisp_cdr(parent);
   lisp* car = lisp_car(parent);
   if (car != (lisp*)NULL){
      print_car(intrp, s, ly);
   }
   while (cdr != (lisp*)NULL){
      intrp->atom_a = 0;
      lisp_reduce(count, car, &intrp->atom_a);
      print_cdr(intrp, g, c, ly);
      car = lisp_car(cdr);
      cdr = lisp_cdr(cdr);
   }
}
...
int set_layer(layer* ly)
{
   //printf("START OF set_layer\n");
   ly->line_index = 0;
   ly->prev_i = 0;
   ly->get_i = 0;
   return 0;
...
void transfer(layer* ly)
{
   //printf("START OF transfer\n");
   ly->width = set_layer(ly);
   for (int i = 0; i < NESTLEN; i++){
      ly->now[i] = ly->prev[i];
   }
}


void print_layer(interpret* intrp, layer* ly, char* s, char* c, char* g)
{
   //printf("START OF print_layer\n");

   while (ly->layers[ly->height][ly->width]){
      lisp* p = ly->layers[ly->height][ly->width];
      print_line(intrp, p, s, c, g, ly);
      ly->width += 1;
   }
   strcat(intrp->istr, "\n");
   ly->width = set_layer(ly);
...
void print_layers(interpret* intrp, layer* ly)
{
   //printf("START OF print_layers\n");

   ly->height = 0;
   ly->width = 0;
   while (ly->layers[ly->height][ly->width]){
      transfer(ly);
      print_layer(intrp, ly, NODE_START, NODE_CONT, NODE_GAP);
      print_layer(intrp, ly, PIP_START, PIP_CONT, GAP);
      print_layer(intrp, ly, V_START, V_CONT, GAP);
      ly->height += 1;
   }
}
...
{
   //printf("START OF intrp_show\n");
   lisp* parent = lisp_fromstring(intrp->args_b[intrp->i]);
   layer* ly = init_layer();
   ly->parent = parent;
   ly->layers[0][0] = parent;
   ly->width = 1;
   while (ly->width){
      next_layer(ly);
   }
   print_layers(intrp, ly);
   if (!intrp->testing){
      printf("%s", intrp->istr);
      intrp->istr[0] = '\0';
   }
...
bool is_iofunc(parser* prsr, interpret* intrp)
{
   ...
   if (!strcmp(prsr->beg, SHOW)){
      push_stack(intrp, show);
      return is_trc_list(prsr, intrp);
   }
   return false;
}
...
void clear_layer(layer* ly)
{
   ly->parent = false;
   ly->height = 0;
   ly->width = 0;
   for (int i = 0; i<NESTLEN; i++){
      for (int j = 0; j<NESTLEN; j++){
         ly->layers[i][j] = false;
      }
   }
}
...
//   320   -5          80    77    101
   ...
   lisp* parent = lisp_fromstring("((1 2) 3 (4 5))");
   layer* ly = init_layer();
   ly->parent = parent;
   ly->layers[0][0] = parent;
   ly->width = 1;
   ...
   char temp[STRLEN] = {'\0'};
   lisp_tostring(ly->layers[0][0], temp);
   assert(!strcmp(temp, "((1 2) 3 (4 5))"));
   lisp_tostring(ly->layers[1][0], temp);
   assert(!strcmp(temp, "(1 2)"));
   lisp_tostring(ly->layers[1][1], temp);
   assert(!strcmp(temp, "3"));
   lisp_tostring(ly->layers[1][2], temp);
   assert(!strcmp(temp, "(4 5)"));
   lisp_tostring(ly->layers[2][0], temp);
   assert(!strcmp(temp, "1"));
   lisp_tostring(ly->layers[2][1], temp);
   assert(!strcmp(temp, "2"));
   lisp_tostring(ly->layers[2][2], temp);
   assert(!strcmp(temp, "4"));
   lisp_tostring(ly->layers[2][3], temp);
   assert(!strcmp(temp, "5"));
   lisp_free(&parent);

   clear_layer(ly);
   parent = lisp_fromstring("(5)");
   ly->parent = parent;
   ly->layers[0][0] = parent;
   ly->width = 1;
   lisp_tostring(parent, temp);
   while (ly->width){
      next_layer(ly);
   }
   lisp_tostring(ly->layers[0][0], temp);
   ...
   clear(prsr, intrp);
   strcpy(prsr->str, "((SHOW '5'))");
   assert(is_prog(prsr, intrp));
   assert(!strcmp(intrp->istr, "   5 \n     \n     \n"));

   clear(prsr, intrp);
   strcpy(prsr->str, "((SHOW '((1 2) 3 (4 5))'))");
   assert(is_prog(prsr, intrp));
   assert(!strcmp(intrp->istr, "   X ---- -> X -> X \n\
   |         |    | \n\
   V         V    V \n\
   X -> X    3    X -> X \n\
   |    |         |    | \n\
   ...
   strcpy(prsr->str, "((SHOW '(5)'))");
   assert(is_prog(prsr, intrp));
   assert(!strcmp(intrp->istr, "   X \n   | \n   V \n   5 \n     \n     \n"));

   clear(prsr, intrp);
   strcpy(prsr->str, "((SHOW '((1 2 (3 4)) 6 (7 8))'))");
   assert(is_prog(prsr, intrp));
   assert(!strcmp(intrp->istr, "   X ---- ---- ---- -> X -> X \n\
   |                   |    | \n\
   V                   V    V \n\
   ...
   clear(prsr, intrp);
   strcpy(prsr->str, "((SHOW '((1 2 (3 4)) (6 1) (7 8))'))");
   assert(is_prog(prsr, intrp));
   assert(!strcmp(intrp->istr, "   X ---- ---- ---- -> X ---- -> X \n\
   |                   |         | \n\
   V                   V         V \n\
   ...
   clear(prsr, intrp);
   strcpy(prsr->str, "((SHOW '((1 2 (3 4)) (6 (1 10)) (7 8))'))");
   assert(is_prog(prsr, intrp));
   assert(!strcmp(intrp->istr, "   X ---- ---- ---- -> X ---- ---- -> X \n\
   |                   |              | \n\
   V                   V              V \n\
